...
export const PROJECTS: Project[] = [
...

  {
    ...
    summary: {
      ru: 'Приложение я написал для себя, после переезда стало нужно считать деньги внимательнее. Оно складывает расходы по категориям, помнит заработок и показывает, сколько можно тратить в день, чтобы дожить до конца месяца. Есть и прогноз на следующий месяц, он считается по вашим же записям за прошлые месяцы.\n\nОтдельная страница собирает цены в магазинах, чтобы понимать, что и где выгоднее закупать. Таблица «товар × магазин» пересчитывает все цены на одну единицу. Пачка 900 грамм за 11,70 считается так, как если бы в ней был целый килограмм, и выходит 13. Килограмм в соседнем магазине стоит 11,10, так что брать надо там. Клетки покрашены по выгоде, от зелёной к красной. Магазин выбирается один раз и запоминается, в зале не нужно каждый раз показывать, где вы стоите.\n\nВсе данные лежат в обычном файле на телефоне. Интернет приложению не нужен, разрешения на него в манифесте нет.',
      en: 'I wrote this one for myself, after moving to another country money needed watching more closely. It adds up spending by category, remembers what came in and shows how much is left to spend per day to reach the end of the month. There is a forecast for the next month too, built from your own records of the months before.\n\nA separate page keeps shop prices, so you can see what is worth buying where. The goods × shops table works every price out per single unit. A 900 gram pack for 11.70 is counted as if it held a whole kilo, which comes to 13. The kilo in the shop next door costs 11.10, so that is where to buy it. Cells are coloured by how good the price is, from green to red. The shop is picked once and remembered, in the aisle you should not have to say where you are standing every time.\n\nEverything sits in an ordinary file on the phone. The app does not need the internet and has no permission for it in the manifest.',
    },
