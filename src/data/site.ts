...
export interface ContactLink {
  id: string;
  /** direct — прямая связь, freelance — биржи, social — соцсети. */
  group: 'direct' | 'freelance' | 'social';
  label: string;
  /** Что показывать под названием: ник, адрес и т.п. */
  handle: string;
  url: string;
  accent: Accent;
}
...
export const PROFILE = {
  ...
  /**
   * E-mail намеренно пустой: адрес из аккаунта не публикуется без прямого
   * распоряжения. Впишите сюда почту для связи — и блок появится на сайте
   * сам. Оставите пустым — раздел просто не отрисуется.
   */
  email: '',
  location: { ru: 'Россия', en: 'Russia' } satisfies Localized<string>,
...
export const ABOUT_TEXT: Localized<string[]> = {
  ru: [
    'Я разработчик полного цикла: беру проект от идеи и до момента, когда его можно открыть, запустить или установить. Мне одинаково интересны игровая механика, вёрстка интерфейса и то, как всё это собирается в готовый продукт.',
  ...
  en: [
    ...
    'What matters to me in the work: a project has to be verifiable. Wherever the logic gets complex — procedural level generation, recipe matching, localisation — I write automated checks, so breakage is found before release rather than after. Bilingual support and a dark theme are designed in from the start, not bolted on at the end.',
  ],
};
