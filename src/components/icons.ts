/**
 * Пути иконок и их тип лежат в .ts, а не в Icon.astro: тип из фронтматтера
 * .astro-компонента не экспортируется — компилятор поднимает объявление
 * наружу и ломает сборку. Здесь же он импортируется откуда угодно.
 *
 * Все иконки нарисованы в сетке 24×24 обводкой (fill="none"), поэтому
 * одинаково смотрятся в любом размере и красятся через currentColor.
 */
