...
const DEMOS = [
  ...
  /*
    ...
    Страницы варианта A перечислены поимённо, и это ловушка: новая страница
    в макете появляется, а сюда её дописать забывают. Тогда меню в копии
    ведёт на файл, которого нет, — при этом на диске у автора всё работает,
    и поломка видна только на выложенной версии. Так уже случилось с пятью
    текстовыми разделами. Варианты B, C и D этой беды лишены: они лежат
    внутри variants/ и копируются папкой целиком.

    Добавили страницу в design/ — допишите её сюда и прогоните
    `npm run demos`.
  */
  {
