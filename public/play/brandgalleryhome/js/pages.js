/*
  Общий слой для текстовых страниц: «О салоне», «Дизайнерам», статьи
  и контакты.

  Почему один файл на четыре варианта. Планировка и оформление у страниц
  разные — это и есть предмет выбора. А вот текст статьи один и тот же:
  если хранить его в разметке каждого варианта, получится четыре копии
  одного абзаца, и первая же правка разъедет их между собой. Поэтому
  здесь лежит содержимое, а варианты дают ему свою обёртку и свои классы.

  Схема проезда тоже здесь. Она нарисована в SVG, а не встроена картой:
  на рабочем сайте это будет интерактивная карта Яндекса (Google Maps
  запрещён правилами проекта — см. CLAUDE.md), а в макете, который
  открывается с диска и не должен делать ни одного внешнего запроса,
  карту показывает схема. Цвета схема берёт у варианта через CSS.

  Скрипт подключается последним, после скрипта самого варианта: к этому
  моменту шапка и подвал уже построены.
*/
(function () {
  'use strict';
  ...
  /* Блок статьи — пара [тип, содержимое]:
       p  — абзац            h  — подзаголовок
       ul — список           q  — врезка-цитата
       note — замечание в рамке

     Разметку внутри абзацев (<b>, <a>) пишем руками: тексты здесь свои,
     из формы посетителя сюда ничего не попадает. */
  var ARTICLES = [
    {
      id: 'divan',
  ...
  /* Появление при прокрутке варианты развешивают один раз при старте,
     а лента статей добавляется уже после — поэтому показываем её сами.
     Без этого карточки остались бы прозрачными навсегда. */
  ...
  /* Страница статьи одна на обе статьи: какая именно — в адресе.
     Так же устроены карточка товара и страница фабрики. */
  function initArticle() {
    ...
    /* Заглушка иллюстрации берётся у варианта: у каждого свой набор
       контуров и свои классы подложки. */
    ...
    /* Оглавление собирается из подзаголовков самой статьи: отдельный
       список пришлось бы править вместе с текстом и однажды забыть. */
  ...
  /* Ленту статей рисуем из тех же данных: список из двух пунктов легко
     набрать руками, но тогда заголовок статьи живёт в двух местах. */
  function initArticleList() {
    ...
    box.innerHTML = ARTICLES.map(function (a, i) {
      ...
      /* Три раскладки под три планировки: карточка (A и D), полоса (B),
         строка списка (C). Разметка разная, данные одни. */
      if (tpl === 'strip') {
      ...
      /* Строка списка: заголовок крупно, тема и время чтения по краям.
         Нужна там, где статьи стоят на длинной главной и не должны
         отнимать у неё целый экран. */
      if (tpl === 'line') {
        return '<a href="' + href + '">' +
    ...
    /* Третья карточка на главной — статья, которая в работе. Она не ссылка:
       страницы ещё нет. Без неё ряд из трёх ячеек обрывается на двух
       и выглядит как ошибка вёрстки. */
  ...
  /*
    Карта нарисована, а не встроена. Причин две.

    Первая — правила проекта: макет не делает ни одного внешнего запроса,
    иначе его нельзя показывать с диска и нечестно говорить «внешних
    адресов нет ни одного». Вторая — на рабочем сайте здесь будет
    интерактивная карта Яндекса, и её размер, поля и подпись должны быть
    известны заранее. Схема занимает ровно то место, которое займёт карта.

    Геометрия соответствует расположению салона: БЦ Full House стоит
    на углу улицы Бабушкина и 2-го Гаражного проезда, второй адрес
    здания — 2-й Гаражный проезд, 4.
  */
  var MAP =
    '<svg viewBox="0 0 1200 560" role="img" ' +
      ...
      /* улицы: широкая горизонтальная — Бабушкина, вертикальная — проезд */
      '<g class="mp-road">' +
        '<path d="M0 305h1200"/>' +
        '<path d="M540 0v560"/>' +
      '</g>' +
      ...
      /* подписи. Название здания стоит на самом здании и потому набрано
         светлым: тот же цвет, что у остальных подписей, на тёмном
         прямоугольнике не читался вовсе. */
      '<g class="mp-label">' +
  ...
  function initMap() {
    $$('[data-map]').forEach(function (el) {
      ...
      /* На телефоне схема шире экрана и прокручивается внутри блока.
         Прокрутка начинается слева, а салон стоит справа от середины —
         в первый момент видно пустой квартал. Поэтому сразу подводим
         метку к центру блока. */
      var svg = el.firstChild;
      if (!svg || el.scrollWidth <= el.clientWidth) return;
      var PIN = 745 / 1200; // доля ширины схемы, на которой стоит метка
      el.scrollLeft = svg.getBoundingClientRect().width * PIN - el.clientWidth / 2;
    });
  }

  /* ------------------------------------------- заглушки и ряд фабрик */

  /* Заглушки на текстовых страницах ставит этот скрипт, а не скрипт
     варианта: там отрисовка привязана к главной, каталогу и карточке
     товара, и добавлять в неё по ветке на каждую новую страницу значило
     бы разложить одно и то же по четырём файлам.

     Атрибут data-ph хранит имя контура; набор контуров у каждого
     варианта свой, поэтому берём его у варианта, а не держим здесь
     свою копию. */
  function initFill() {
    ...
    /* Строка марок варианта B: та же лента, что стоит над каталогом.
       Отдельная от сетки логотипов варианта A — это разная вёрстка
       под разные планировки, а не одна с двумя наборами классов. */
  ...
  /* Формы на этих страницах ничего никуда не отправляют: макет
     статический. Задача — показать, что происходит после нажатия,
     иначе на показе кнопка выглядит нерабочей.

     Отдельно от заявки в подборке: та живёт в скрипте варианта и умеет
     ещё и подставлять отмеченные товары. */
  function initForms() {
